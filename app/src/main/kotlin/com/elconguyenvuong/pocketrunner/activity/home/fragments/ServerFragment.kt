...
import com.elconguyenvuong.pocketrunner.ServerService
import com.elconguyenvuong.pocketrunner.server.*
...
class ServerFragment : BaseFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        pharService.setText(HtmlCompat.fromHtml(getString(R.string.phar_service), HtmlCompat.FROM_HTML_MODE_LEGACY))
        BetterLinkMovementMethod.linkifyHtml(pharService)
        basedOn.setText(HtmlCompat.fromHtml(getString(R.string.based_on), HtmlCompat.FROM_HTML_MODE_LEGACY))
        BetterLinkMovementMethod.linkifyHtml(basedOn)

        dataSet = LineDataSet(ArrayList<Entry>(), null)
        dataSet.setDrawValues(false)
        dataSet.mode = LineDataSet.Mode.CUBIC_BEZIER
        dataSet.color = ContextCompat.getColor(requireContext(), R.color.primaryLightColor)
    ...
    private val statUpdateObserver = ServerBus.listen(UpdateStatEvent::class.java).subscribe ({
        ...
        requireActivity().runOnUiThread {
            ...
            val statics: List<String> = it.state.entries.map { "${it.key} ${it.value}" }
